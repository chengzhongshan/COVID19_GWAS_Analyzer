...
*/
create_eqtl_boxplots=0,/*Provide value 1 to make boxplot for each SNP;
...
title "eQTL boxplot for &_snp_";
data snp_exp&_bs_;set &genoexp_outdsd;
*Only keep target snp and tissues with eqtl p < cutoff;
where SNP="&_snp_";
tissue=prxchange('s/^[^_]+_//',1,tissue);
tissue=prxchange('s/_/ /',-1,tissue);
