%macro Lattice_gscatter_over_bed_track0(/*Old macro without adding of gene annotation to tracks, which is for backup only*/
...
/*																								linearopts=(minorticks=false tickvaluelist=(&y_axis_values) )*/
                      linearopts=(minorticks=false tickvaluelist=(&y_axis_values) tickdisplaylist=(&ylabelsmacro_var))
                        ...
                        xaxisopts=(linearopts=(viewmin=&min_x viewmax=&max_x) offsetmin=0.05 offsetmax=0.05);
...
run;
ods graphics /imagename="&yval_var.&chr_var.:&st_var.-&end_var.png" noborder;
...
%Lattice_gscatter_over_bed_track0(
bed_dsd=x0,
chr_var=chr,
st_var=st,
end_var=end,
