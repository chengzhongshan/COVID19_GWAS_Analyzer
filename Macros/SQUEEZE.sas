...
* NOTE:    length of format for char vars is changed to match computed length
* of char var
...
* NOTE:    variables named in &NOCOMPRESS are not included in the minimum-
*  length computation process and keep their original lengths as specified in
* &DSNIN
*
* EXAMPLE OF USE:
...
/*===============================================================================*/
 ...
 WHERE (TYPE =1 AND LENGTH ^= 3) OR (TYPE =2 AND LENGTH ^=1);
  if nobs = 0 then stop ; n_char + ( type = 2 ) ; n_num +(type=1);
/* create macro vars containing final # of char, numeric variables */
 if lastobs then do ;
  end ;
  call symput( 'N_CHAR', left( put( n_char, 12. ))) ; 
  call symput( 'N_NUM' , left( put( n_num , 12. ))) ;
 run ;
 /*===============================================================================*/
 /* if there are NO numeric or character vars in dataset, stop further            */
 /* processing                                                                    */
 /*===============================================================================*/
 %if %sysevalf( &N_NUM + &N_CHAR, boolean ) %then %do ;
 %put the table has enough vars for the macro;
 %end;
 %else %do;
 %put /----------------------------------\ ;
 %put | ERROR from SQUEEZE:              | ;
...
%let SQZ_CHAR&I     = length %qtrim( &&CHAR&I ) $%left( &&CHARLEN&I ) ;
%let SQZ_CHAR_FMT&I = format %qtrim( &&CHAR&I ) $%left( &&CHARLEN&I ). ;

%end ;

%if &N_NUM > 0 %then %do I = 1 %to &N_NUM ;
/*     %let SQZ_NUM = &SQZ_NUM %qtrim( &&NUM&I ) &&NUMLEN&I ; */
*To avoid of truncating long strings of variables, create single macro var;
       %let SQZ_NUM&I = length %qtrim(&&NUM&I) &&NUMLEN&I;
%end ;
/*===============================================================================*/
...
data _null_ ;
dsid = open( "&DSNIN", 'I' ) ; /* open dataset for read access only */
call symput( 'totvars', attrn( dsid, 'nvars' )  ) ;
do _i_ = 1 to attrn( dsid, 'nvars');
 call symputx('V'||left(put(_i_,12.)),varname(dsid,_i_));
end;
rc=close(dsdid);
run ;
%let totvars=%left(&totvars);
/*===============================================================================*/
