...
/* Demonstration of data analysis with HpmixArray                          */
/*                                                                        */
/*========================================================================*/
...
%include "F:\360yunpan\SASCodesLibrary\SAS-Useful-Codes\Macros\HpmixArray.sas";

x cd "F:\360yunpan\SASCodesLibrary\SAS-Useful-Codes\es\MT_lab(SNU)\AnovArray\AnovArray.1.1";
...
/*  -----------                                                        */
/*                                                                     */
/*  The macro uses proc glm to fit complete analysis of variance     */
...
/*  -------                                                            */
/*                                                                     */
/*  Syntax for this macro is similar to that of proc glm.            */
/*  However some options of proc glm are not available here.         */
/*                                                                     */
...
/*                                                                     */
/*  procopt     -- OPTIONAL -- by default no proc glm option --      */
/*              specifies the options appropriate for proc glm       */
/*              statement.                                             */
/*                                                                     */
/*  stmts       specifies proc glm statements for the analysis,      */
...
/*                                                              */
/*    %anova                                                    */
/*    Calculate factors contribution using proc glm           */
/*    old code deprecated                                      */
/*--------------------------------------------------------------*/

%macro anova;
*proc glm and hpmixed have used to replace proc anova;
...
/*Can not use proc glm with outstat directly, as sas will be out of memory*/
...
/*Use proc hpmixed or proc anova but not proc glm*/

/*proc glm data=_sortbygene outstat=_VARGENEGLOB noprint;*/
/*  class gene;*/
/*  model &response=gene;*/
/*run;*/

/*ods select none;*/
/*ods output OverallANOVA =_VARGENEGLOB;*/
/*proc hpmixed data=_sortbygene;*/
/*  class gene;*/
/*  model &response=gene;*/
/*run;*/
/*ods select all;*/
...
%put Wait a few minutes, proc hpmixed is in progress.....;
%put;
...
/*    run;*/
					...
					*Get signal std by treatment;
						proc sql;
						create table _gene_std as
						select std(&signal) as exp_std,gene,&treatment
						from _dataset
      group by gene, &treatment
      order by gene,&treatment;
						create table _graph_new as 
						select a.*,b.exp_std,catx("_",b.&treatment,"std") as grp4std
						from _graph as a,
						     _gene_std as b
						where a.gene=b.gene and 
						      a.&treatment=b.&treatment;
						quit;
				*Make output dsd include mean exp of different treatments;
						%let vars=gene sstot dfgene sigma2gene fhet phet result_het PhetAdjusted;
      proc sort data=_graph_new;by &vars;
						proc transpose data=_graph_new out=trans_grp_std(drop=_name_);
						  var exp_std;
						  id grp4std;
						  by &vars;
						run;
      proc transpose data=_graph_new out=trans_graph(drop=_name_);
        var mn&treatment.gene;
        id &treatment;
        by &vars;
      run;
						proc sql;
						create table &outdata as 
						select * from trans_graph
						natural join
						trans_grp_std;
						quit;
