...
if (_n_=1) then do;
 ...
 do while(scan(_infile_,i,&dlm) ne "");
  ...
  y=prxchange('s/["]+//',-1,y);	/*remove " in headers*/
...
infile "&file" dsd dlm=&dlm truncover lrecl=32767 firstobs=&dataline obs=%eval(&firstobs+&rows) end=eof;
input;
/*Calcuate total number of vars*/
 i=1;
...
infile "&file" dsd dlm=&dlm lrecl=32767 truncover firstobs=&dataline obs=%eval(&firstobs+&rows);
...
%ImportPartialFilebyScan(file=TCGA-59-2355-01A-01R-1569-13.cd.bed
,dsdout=dsd
,firstobs=0
,rows=100
,dlm='09'x
