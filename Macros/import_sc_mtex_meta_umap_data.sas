...
mean_exp_cutoff=0, /*Only keep row records with mean expression > mean_exp_cutoff*/
max_cells2import=1000000 /*Maximum cells to be imported into sas; if the total number of cells 
in the cell matrix is greater than the cutoff, the macro will randomly select the maximum number 
of cells from the cell matrix*/
);
...
*Add the mean gene expression filter fo the extra_cmd4infile;
%if %length(&target_genes)>0 %then %let extra_cmd4infile=%nrbquote(if prxmatch("/(&target_genes)/i",rownames) and mean(of V:)>&mean_exp_cutoff);
%else %let extra_cmd4infile=%bquote(if mean(of V:)>&mean_exp_cutoff);
...
max_cells2import=&max_cells2import
);
/* %abort 255; */
