...
If you do not want to cluster the data and just need to generate a heatmap,
please use the macro heatmap4longformatdsd and Rename_Del_Rgx4All_Vars!
*/
...
numvars_label_dsd=,/*Default is empty, which will use the names of these numeric vars to label heatmap;
...
numvarname_in_labeldsd=numgrp,/*variable name for these numeric variable names in the label dsd, which contains values such as V1, V2, V3, and so on*/
numvarlabel_in_labeldsd=wide_ids,/*the variable name containing the new labels for these numeric vars in the label dsd*/
...
/*Important: double quote is required to assign font family for the column and row axes in the following two parameters!*/
colaxis_font_setting=%str(Style=italic size=10 weight=normal family="Couriers"),/*column label setting for font style, size, and weight, as well the font family with even space*/
rowaxis_font_setting=%str(Style=italic size=10 weight=normal family="Couriers"),/*column label setting for font style, size, and weight, as well the font family with even space*/
...
Note: the final rowlabels will be rotated regardless the swithing row or column names;*/
max_rows4labeling=200,/*If there are >200 rows in the heatmap, row labels will be supressed for clearity and simplicity*/
max_cols4labeling=200,/*If there are >200 columns in the heatmap, column labels will be supressed for clearity and simplicity*/
...
To eliminate left most column-wide cell or upper row-wide cell when performing cluster with type value as 1 or 2, please see the notes for the
using specific settings for the two parameters columnweights and rowweights*/
...
which are hardcoded internally in the macro, and also ensure the two parameters name and attrmap use the same quoted string;
...
set Longformat_heatmap_dsd;
ord=_n_;
run;

*Modify the heatmap values of longformat_heatmap and keep its row order by sorting with ord;
*The following codes has been included into the macro, and it is kept here for backup purpose!;
*Note: the heatmap will be changed for values but the clusters on row-wide and column-wide will be remain unchanged!;
data tgt;
set longform_heatmap;
*Your codes to modify the values of color variable Dist;
run;

*It is also possible to use different column or row labels for making a updated heatmap as follows:;
proc sql;
create table tgt as
select a.*,b.wide_ids as _col_
from longformat_heatmap_dsd as a
left join
Wide_ids_lookup as b
on a.col=b.numgrp
order by ord;
create table tgt as
select a.*,b.wide_ids as _col_1
from tgt as a
left join
Wide_ids_lookup as b
on a.x_name_=b.numgrp
order by ord;
data tgt(rename=(x_name_1=x_name_));
set tgt(drop=col);
length x_name_1 $100.;
rename _col_=col;
x_name_1=x_name_;
if _col_1^="" then x_name_1=_col_1;
drop x_name_;
run;
...
*Use new labels for column or row variables in the heatmap;
%if %length(&numvars_label_dsd)>0 %then %do;
%let totcolnames=%totobsindsd(&numvars_label_dsd);
proc sort data=&numvars_label_dsd nodupkeys;
by &numvarlabel_in_labeldsd &numvarname_in_labeldsd;
run;

%let uniq_totcolnames=%totobsindsd(&numvars_label_dsd);
%if &totcolnames>&uniq_totcolnames %then %do;
					 %put There are unique combinations of &numvarlabel_in_labeldsd and &numvarname_in_labeldsd in the input data set &numvars_label_dsd;
           %put which has &numvars_label_dsd records including duplicated records;
           %put Please ensure the input lookup data set not contain duplicates by &numvarlabel_in_labeldsd and &numvarname_in_labeldsd;
           %abort 255;
%end;
 ...
 %end;
    ...
    *Note: sort the dataset by col will affect the final heatmap rowwide order;
    *So it is possible to manually sort the heatmap by adding a customized variable;
/*   
    proc sort data=&heatmap_dsd;by &_colvar_;run;
    data &heatmap_dsd;
    set &heatmap_dsd;
    ord=_n_;
    run;
    ...
    proc sql;
    create table &heatmap_dsd as
    select a.*,b.&numvarlabel_in_labeldsd as _&_colvar_,b.
    from &heatmap_dsd as a
    left join
    &numvars_label_dsd as b
    on a.&_colvar_=b.&numvarname_in_labeldsd
    order by ord;
  */
     proc sql;
    create table &heatmap_dsd as
    select a.*,b.&numvarlabel_in_labeldsd as _&_colvar_,b.&num_ord_var
    from &heatmap_dsd as a
    left join
    &numvars_label_dsd as b
    on a.&_colvar_=b.&numvarname_in_labeldsd
    order by b.&num_ord_var;

    create table &heatmap_dsd as
    select a.*,b.&numvarlabel_in_labeldsd as _&_colvar_.1
    from &heatmap_dsd as a
    left join
    &numvars_label_dsd as b
    on a.&_axis_name._name_=b.&numvarname_in_labeldsd
    order by  &num_ord_var;

/*	%abort 255;*/

    data &heatmap_dsd(rename=(&_axis_name._name_1=&_axis_name._name_));
    set &heatmap_dsd(drop=&_colvar_);
    length &_axis_name._name_1 $200.;
    rename _&_colvar_=&_colvar_;
    &_axis_name._name_1=&_axis_name._name_;
    if _&_colvar_.1^="" then &_axis_name._name_1=_&_colvar_.1;
    drop &_axis_name._name_;
    run;
%end;
/*%abort 255;*/
...
%if &nrows< &max_rows4labeling %then %do; 
...
%if &ncols< &max_cols4labeling %then %do; 
...
/*      begingraph / designheight=&height.cm designwidth=&width.cm;*/
    begingraph / designheight=&height  designwidth=&width;
        %if %length(&rangemap_setting)>0 %then %do;
              &rangemap_setting;  
         %end;
		 *Note: borderattr=(color=white) will remove the frame lines for the whole figure;
         layout lattice    / rowdatarange=union columndatarange=union
                             rows=2 columns=2 	borderattrs=(color=white)
            ...
            *To remove outline, add "walldisplay=none";
            layout overlay / walldisplay=none 
                             ...
                             xaxisopts=(
                                           display=(&rowaxis_label_setting) TICKVALUEATTRS=(&rowaxis_font_setting)
...
*To achieve even spacing (monospaced font) in SAS, it is necessary to use a monospaced font by setting it with goptions ftext;
*allocate the same amount of horizontal space for each character, regardless of its width. Examples include Courier, Menlo, and Consolas.;
goptions ftext=Courier;

*Need to exclude missing values;
proc sgrender data=&heatmap_dsd template=HeatDendrogram;
run;
