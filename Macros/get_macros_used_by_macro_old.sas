%macro get_macros_used_by_macro_old(
macrorgx=.,
...
if prxmatch("/^(\s)?\%([^\s\(])+\(/",Macro_Info) then do;
...
%do mxi=1 %to &totmacros;
        %let _submacro_=&&submacro&mxi;
        %put You first level submacro: &_submacro_;
	*Now perform repeat searching for 100 time and stop if no increasement of the dataset macros;

	data macros_&mxi;
	length file $1000. Macro_Info $1000.;
	infile "&_submacro_" lrecl=10000 obs=max;
	input;
	Macro_Info=_infile_;
	file="&_submacro_";
	*Use regex to parse lines in macro for submacros used by the macro;
	if prxmatch("/^(\s)?\%([^\s\(])+\(/",Macro_Info) then do;
	 Macro_Info=trim(left(prxchange("s/(^.*[\/\\])[^\/\\]+.sas/$1/i",-1,file))) || trim(left(prxchange("s/^(?:\s?)\%([^\s\(]+)\(.*/$1/",-1,Macro_Info))) || ".sas";
	 stage=2;
	 if Macro_Info^=file then output;
	 end;
	run;
        
        data _macros&i;
        set _macros&i macros_&mxi;
        run;
%end;
